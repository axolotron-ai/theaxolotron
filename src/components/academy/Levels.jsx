...
export default function Levels() {
    ...
    const value = [{ src: "/students.jpg", name: "Students" }, { src: "/employees.jpg", name: "Employees" }, { src: "/organizations.jpg", name: "Organization" }];

    return (
        <>
            <div id="academiclevels" className="bg-[#DDDDDD] px-[3%] py-[5%] flex flex-col gap-[40px]">
                <div>
                    ...
                    <div className="flex flex-wrap gap-[25px] items-center justify-between mx-[10%]">
                        {
                            value.map((val, index) => (
                                <div key={index} className="relative flex flex-col gap-[20px]">
                                    ...
                                    <div className="absolute bottom-0 bg-gradient-to-t text-white text-[20px] p-[10px] font-semibold from-black via-black/70 to-transparent flex items-center justify-between w-full">AI for {val?.name} <ArrowRight className="text-black bg-white h-[40px] w-[40px] p-[10px] rounded-full" /></div>
                                </div>
                            ))
                        }
                    </div>
